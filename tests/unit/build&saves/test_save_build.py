...
def test_remove_saved_build():
    ...
    builds_repo.save_build(build_id, user['user_id'][0])
    user_builds = builds_repo.get_all_saves_from_user_id(user['user_id'][0])

    found_save = False
    for save in user_builds:
        found_save = save['build_id'] == build_id

    builds_repo.remove_saved_build(build_id, user['user_id'][0])
    builds_repo.for_testing_only_delete_user("Example1")
